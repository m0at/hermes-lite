#!/usr/bin/env python3
"""
hermes-lite CLI - Interactive Terminal Interface

A stripped-down command-line interface for hermes-lite, inspired by Claude Code.
...
# Point mini-swe-agent at ~/.hermes-lite/ so it shares our config
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.hermes-lite/.
...
def _parse_reasoning_config(effort: str) -> dict | None:
    """Parse a reasoning effort level into a reasoning config dict.

    Valid levels: "xhigh", "high", "medium", "low", "minimal", "none".
    Returns None to use the default, or a config dict to override.
    """
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.hermes-lite/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.
    """
    # Check user config first (~/.hermes-lite/config.yaml)
    ...
    defaults = {
        ...
        "compression": {
            ...
            "summary_model": "claude-haiku-4-5",  # Fast/cheap model for summaries
        },
        "agent": {
...
COMPACT_BANNER = """
[bold #FFD700]╔══════════════════════════════════════════════════════════════╗[/]
[bold #FFD700]║[/]  [#FFBF00]⚕ HERMES-LITE[/] [dim #B8860B]- Local Coding Agent[/]              [bold #FFD700]║[/]
[bold #FFD700]║[/]  [#CD7F32]Anthropic-first CLI[/]    [dim #B8860B]optional local Qwen[/]      [bold #FFD700]║[/]
[bold #FFD700]╚══════════════════════════════════════════════════════════════╝[/]
"""
...
def build_welcome_banner(console: Console, model: str, cwd: str, tools: List[dict] = None, enabled_toolsets: List[str] = None, session_id: str = None, context_length: int = None):
    ...
    left_lines.append(f"[#FFBF00]{model_short}[/]{ctx_str} [dim #B8860B]·[/] [dim #B8860B]hermes-lite[/]")
    ...
    right_lines.append("")
    right_lines.append(f"[dim #B8860B]{len(tools)} tools · /help for commands[/]")
    ...
    outer_panel = Panel(
        layout_table,
        title=f"[bold #FFD700]hermes-lite {VERSION}[/]",
        border_style="#CD7F32",
        padding=(0, 2),
    )
...
COMMANDS = {
    ...
    "/provider": "Switch provider (anthropic or local)",
...
# hermes-lite does not expose dynamic skill slash commands.
_skill_commands = {}


class SlashCommandCompleter(Completer):
    """Autocomplete for built-in slash commands in the input area."""
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.hermes-lite/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.hermes-lite/config.yaml on first use)
...
class HermesCLI:
    """
    Interactive CLI for hermes-lite.
    ...
    def __init__(
        ...
        Args:
            ...
            provider: Inference provider ("anthropic" or "local")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.hermes-lite/images/ and appends the path to
    ...
    def show_help(self):
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with hermes-lite!{_RST}")
    ...
    def _handle_provider_command(self, cmd: str):
        ...
        try:
            if arg == "anthropic":
                ...
                print(f"  Switched to Anthropic: {model}")
    ...
    def _handle_cron_command(self, cmd: str):
        """Cron is intentionally excluded from hermes-lite."""
        self.console.print("[bold yellow]Scheduled tasks are not included in hermes-lite.[/]")

    def _handle_skills_command(self, cmd: str):
        """Skills Hub is intentionally excluded from hermes-lite."""
        self.console.print("[bold yellow]Skills Hub is not included in hermes-lite.[/]")

    def _show_gateway_status(self):
        """Messaging gateway is intentionally excluded from hermes-lite."""
        self.console.print("[bold yellow]Messaging gateway support is not included in hermes-lite.[/]")
    ...
    def process_command(self, command: str) -> bool:
        ...
        if cmd_lower in ("/quit", "/exit", "/q"):
            ...
            if len(parts) > 1:
                ...
                # Resolve Claude shortcuts against the direct Anthropic models only.
                claude_alias = _CLAUDE_MODELS.get(arg_lower)
                if claude_alias:
                    ant_key = os.getenv("ANTHROPIC_API_KEY", "").strip()
                    if ant_key:
                        new_model = claude_alias if isinstance(claude_alias, str) else claude_alias[0]
                        self.requested_provider = "anthropic"
            ...
            if base_cmd in {"/reload-mcp", "/copycode", "/swarm", "/cron", "/skills", "/platforms"}:
                self.console.print(
                    "[bold yellow]That command is not included in hermes-lite.[/]"
                )
                self.console.print(
                    "[dim #B8860B]This fork keeps the local coding CLI and drops gateway, skills-hub, swarm, and MCP extras.[/]"
                )
                return True
    ...
    def _reload_mcp(self):
        """MCP servers are not included in hermes-lite."""
        self.console.print("[bold yellow]MCP is not included in hermes-lite.[/]")

    def _copycode(self):
        """Skill import is not included in hermes-lite."""
        self.console.print("[bold yellow]Skill import is not included in hermes-lite.[/]")

    def _handle_swarm_command(self, command: str):
        """Swarm mode is not included in hermes-lite."""
        self.console.print("[bold yellow]Swarm mode is not included in hermes-lite.[/]")
    ...
    def run(self):
        ...
        self.show_banner()
        self.console.print("[#FFF8DC]Welcome to hermes-lite. Type your message or /help for commands.[/]")
        self.console.print()
...
def main(
    ...
    """
    hermes-lite CLI - Interactive AI Assistant
    ...
    Examples:
        ...
        python cli.py --toolsets hermes-lite-cli # Use the default lite toolset
        python cli.py -q "Summarize this repo"   # Single query mode
    ...
    # hermes-lite intentionally excludes gateway mode.
    if gateway:
        print("Messaging gateway mode is not included in hermes-lite.")
        return
    ...
    # Default to the hermes-lite CLI toolset
    toolsets_list = None
    if toolsets:
        ...
        # Check config for CLI toolsets, fallback to hermes-lite-cli
